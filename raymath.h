...
#define RMAPI                                                                                      \
    __declspec(dllexport) extern inline  // We are building raylib as a Win32 shared library (.dll)
#elif defined(BUILD_LIBTYPE_SHARED)
#define RMAPI                                                                                      \
    __attribute__((                                                                                \
        visibility("default")))  // We are building raylib as a Unix shared library (.so/.dylib)
...
RMAPI void Vector3OrthoNormalize(Vector3* v1, Vector3* v2)
{
    ...
    Vector3 vn1 = { v1->y * v2->z - v1->z * v2->y,
                    v1->z * v2->x - v1->x * v2->z,
                    v1->x * v2->y - v1->y * v2->x };
    ...
    Vector3 vn2 = { vn1.y * v1->z - vn1.z * v1->y,
                    vn1.z * v1->x - vn1.x * v1->z,
...
RMAPI Vector3
Vector3CubicHermite(Vector3 v1, Vector3 tangent1, Vector3 v2, Vector3 tangent2, float amount)
{
    Vector3 result = { 0 };
...
// as described in the GLTF 2.0 specification:
// https://registry.khronos.org/glTF/specs/2.0/glTF-2.0.html#interpolation-cubic
RMAPI Quaternion QuaternionCubicHermiteSpline(
...
RMAPI Quaternion QuaternionFromVector3ToVector3(Vector3 from, Vector3 to)
{
    ...
    float cos2Theta
        = (from.x * to.x + from.y * to.y + from.z * to.z);  // Vector3DotProduct(from, to)
